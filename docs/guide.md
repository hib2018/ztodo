...
- `Tab`：Tasks、Issueツリー、Issue本文、Activityの順にフォーカスを切り替える
...
Activityペインは通常、枠を含む4行で直近の操作と結果を1件表示します。フォーカス中は8行へ広がり、直近3件を表示します。`j` / `k`または`↓` / `↑`でセッション内の履歴をスクロールし、`q`でTasksへ戻ります。履歴は最大32件で、TUI終了時に破棄されます。Issue本文やClipboardの内容などは記録せず、安全な操作名と結果の要約だけを表示します。
